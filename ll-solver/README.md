...
1. **Trace the solution.** The BFS from Stage 1 doesn't store the actual moves — it only records how many slides each position is from the goal. To recover the moves, the solver starts at the puzzle's initial position and works forward: it tries every possible slide (each robot × each direction), and checks which ones land on a state whose distance is exactly one less (one slide closer to solved). That move must be on a shortest path, so the solver takes it and repeats from the new position until the goal is reached. At each step, the solver knows exactly who moved, which direction, and who blocked them — because it tried the move explicitly. It records these as a triple: *(who moved, direction, who stopped them)*. For example, "the exit robot slid up and was stopped by helper 3" becomes (exit, up, helper-3). The full sequence of triples — ignoring the specific cells and distances — captures the *strategic structure* of the solution: which robots interact, in what order, and from which directions. Two puzzles where the robots are in completely different places can produce the exact same sequence of triples, meaning a player would experience them as the same puzzle.
