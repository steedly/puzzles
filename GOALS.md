...
- **Solutions must be provably optimal.** The enumerator's 4-stage pipeline (retrograde BFS → collision-sig dedup → compaction → DP trace) guarantees that `minMoves` is the true grouped-move optimum. The UI solver must match this when recomputing. The distinction between individual slides and grouped moves (consecutive slides of the same robot = 1 grouped move) is critical — players experience grouped moves, not slides. Historical bug: the DP trace's "same mover" check compared sorted helper indices instead of physical robot identities — this produced suboptimal grouped-move counts for puzzles where helper sorting changed the index mapping between consecutive moves. Fixed by tracking movers by cell position instead of index.
...
- **Variant hierarchy is monotone.** Blocked cells only prevent moves, never enable them — so the set of solvable positions on a more-restricted board is always a subset of the less-restricted board. This means variant bugs are more likely to manifest as *missing* puzzles on restricted variants (a puzzle solvable on French should also be solvable on Standard). If a restricted variant produces a puzzle not found on a less-restricted variant, that indicates a bug in canonicalization or dedup, not a genuine new puzzle.
...
- Always generate all four variants when updating puzzle data.
...
- Current deployed puzzle files (4 exits, ≤6 total): Standard (~8.5 MB, 243K puzzles), French (~3.5 MB, 103K puzzles), Solitaire (~1.5 MB, 45K puzzles), UFO (~1.0 MB, 30K puzzles). Variant files are lazy-loaded; only Standard is fetched on initial page load.
- Full 7-total-robot files are saved in `ll-solver/full/` (untracked) for future filtering work: Standard (437 MB, 10.3M puzzles), French (TBD), Solitaire (16 MB, 402K), UFO (10 MB, 245K).
